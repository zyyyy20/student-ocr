...
def build_docx() -> None:
    ...
    add_body_paragraph(
        ...
        "主案例固定为仓库根目录下的「IMG_20260412_225722.jpg」（班级平时成绩表手机拍照，含轻微透视、倾斜与留白，便于观察各步效果）。"
        "若需更换案例，可替换该文件或修改脚本中 PRIMARY_CASE 后重新运行脚本；亦可按章首「案例图准备建议」增补多类脱敏样例。",
    )
    add_thesis_figure(
        doc,
        FIG4_1,
        "图4-1　4.1 节：解码与预处理入口前后对照（案例：IMG_20260412_225722.jpg；图中为中文标注）",
        width=Inches(5.2),
    )
        ...
        """// 典型成功响应（字段视样本略有增减）
{
  ...
  "processed_preview": "data:image/png;base64,..."
    ...
    add_body_paragraph(
        doc,
        "为便于答辩材料与论文插图一致，可用与第4章相同的案例图「IMG_20260412_225722.jpg」作为代表性 A 类（教务拍照类）样本，经与本系统一致的 preprocess_with_context 得到线上 /upload 实际输入。"
