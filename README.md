...
1. 先识别“图上有哪些文字、它们在哪”
2. 再判断“这些文字在表格里属于哪一行哪一列”
3. 最后修正“漏掉的小数字、拆开的行、错位的单元格”
...
也就是说，OCR 在这里不仅是“读文字”，还是后续结构恢复的基础观测数据。
...
如果只做前者，不做后者，就很容易把同一行拆开、把同一列错位，或者把页脚说明误并入表格。
...
本质上这是一个基于几何分布的轻量结构恢复方法：

- y 方向相近，认为属于同一行
- x 方向相近，认为属于同一列

这种方法对规则截图非常有效，因为截图中的行列通常排列整齐。
...
这一步的核心原理类似“先粗定位，再细识别”：

- 第一次先知道成绩单大概在哪、表格大概在哪
- 第二次只在更小、更干净的区域里识别

这样可以减少背景、空白大表格、页脚说明对识别的干扰。
...
- 整图 OCR 更适合整体扫描
- 但短数字、小数、靠近表格线的值容易漏掉
- 所以把目标单元格单独裁出来，再放大识别，能显著提高这类数字的召回率
...
这是当前系统里最贴近“识别纠错”的一层，不是简单规则替换，而是把漏掉的数字重新送回 OCR。
